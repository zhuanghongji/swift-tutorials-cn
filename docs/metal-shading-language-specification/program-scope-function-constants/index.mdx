...
你可以使用 [[function_constant(name)]] 属性声明图形、内核或其它用户函数的参数，以标识该参数是可选的。name 属性指的是函数常量变量。如果名称给出的函数常量变量的值非零或为真（在创建管道状态期间确定），则参数的声明位于函数签名中。如果名称给出的函数常量变量的值为 0 或 false，则该参数不会在函数签名中声明。如果 name 引用尚未定义的函数常量变量（在创建管道状态期间确定），则行为与 is_function_constant_define(name) 的值为 false 时相同。
