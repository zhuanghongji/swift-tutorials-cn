...
struct rasterization_rate_map_decoder { 显式 rasterization_rate_map_decoder( 常量 rasterization_rate_map_data &data) 线程;

float2 地图屏幕到物理坐标(float2 屏幕坐标, uint layer_index = 0) const 线程; uint2 map_screen_to_physical_coordinates(uint2 screen_coordinates, uint layer_index = 0) const thread; float2 map_physical_to_screen_coordinates(float2physical_coordinates, uint layer_index = 0) const thread; uint2 map_physical_to_screen_coordinates(uint2physical_coordinates, uint layer_index = 0) const thread; };
...
[[fragment]] float4 fragment*shader(/* 其它参数 \_/ 常量 rasterization_rate_map_data &data [[buffer(0)]]) { float2 screen_coords = ...; rasterization_rate_map_decoder 映射（数据）； float2physical_coords = map.map_screen_to_physical_coords(screen_coords); ... }
