...
``` border_color macOS：Metal 1 .2。 iOS：金属 2.3。 ````
...
钳位到零相当于钳位到边界，其边框颜色为透明黑色（0.0，0.0，0.0），并且具有来自纹理的 alpha 分量值。如果 clamp_to_zero 是一个或多个纹理坐标的地址模式，则当边框颜色为 transparent_black 时，其他纹理坐标可以使用 clamp_to_border 的地址模式。否则，行为是未定义的。
