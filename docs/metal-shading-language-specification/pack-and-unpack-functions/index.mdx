...
<OrigninalPDF title="6.14 Pack and Unpack Functions" />

本节列出了头文件 中定义的 Metal 函数，用于将向量浮点数据与压缩整数值相互转换。有关如何从 8 位、10 位或 16 位有符号或无符号整数值转换为标准化单精度或半精度浮点值（反之亦然）的详细信息，请参阅第 7.7 节的小节。
