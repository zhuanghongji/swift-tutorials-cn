...
对于 arm64 架构，请始终使用音频组件 API 将音频单元、编解码器和其他代码模块加载到你的应用中。音频组件 API 是搜索可加载代码模块的现代方法，可在 macOS 10.6 及更高版本中使用。面向 arm64 架构或链接到 macOS 11 SDK 的应用程序无法使用旧版 Carbon Component Manager API 来打开音频单元。如果你的应用程序使用 Carbon 组件管理器，请计划在移植应用程序时将其迁移出去。
...
每个核心音频设备都提供一个工作组，其他实时线程可以使用 os_workgroup_join_self 函数加入该工作组。加入音频设备工作组会告诉系统你的应用程序的实时线程正在努力争取与设备线程相同的截止日期。你可以通过以下几种方式之一访问与设备关联的工作组：
