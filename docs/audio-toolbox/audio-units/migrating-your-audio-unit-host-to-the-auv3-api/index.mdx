...
<OriginalSource
  ...
  summary="更新您的音频单元 (AU) 主机应用程序以利用 AUv3 的新特性和功能"
/>
...
您可以采用新的 API，同时仍然保留托管 AUv2 音频单元的能力。该框架的桥接层通过自动将 AUv3 调用转换为其 AUv2 等效项来实现此功能，这意味着您可以使用单个 API 并使用新的和旧的音频单元。
...
AVFoundation 框架的 AVAudioUnitComponentManager 类提供了一种查找在主机系统中注册的音频组件的便捷方法。您可以使用它通过描述、谓词或测试用例搜索音频单元，并且组件管理器返回与您的搜索条件匹配的 AVAudioUnitComponent 对象数组。以下代码示例展示了如何使用每种方法来查找系统上可用的所有音频效果。
...
找到可用音频组件的列表后，下一步就是实例化您要使用的组件。要实例化音频单元，请使用 AVAudioUnit 类的 instantiate(with:options:completionHandler:) 方法，如下所示。
...
两个音频单元 API 版本之间的主要区别在于 AUv2 音频单元加载到主机进程中，而 AUv3 音频单元默认加载到进程外。仅在 macOS 中，如果 AUv3 音频单元支持，您可以通过在 options 参数中传递 loadInProcess 选项来请求在进程中加载 ​​ 它。要验证音频单元是否已在进程中成功加载，请查询其 isLoadedInProcess 属性。

:::

## 与音频单元交互 in page link

使用 AUv3 API 与音频单元交互的方式不同于使用 AUv2 API 的方式。在 AUv2 中，您使用 C 函数来操作音频单元。例如，以下代码设置使用 AUv2 API 渲染的最大帧数。
...
相比之下，AUv3 API 提供了更自然的接口，使您能够通过调用音频单元实例的属性和方法来直接与音频单元实例交互。使用 AUv3 API，您可以设置要渲染的最大帧数，如下所示。
...
## 展示音频单元的用户界面 in page link

要显示音频单元的用户界面，请调用其 requestViewController(completionHandler:) 方法。此方法异步检索音频单元的视图控制器实例并在回调闭包中返回它。回调是在后台线程上调用的，因此您需要在将音频单元的用户界面添加到应用程序的视图层次结构之前将控制分派回主队列。
