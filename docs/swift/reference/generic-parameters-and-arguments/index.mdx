...
泛型参数由一个类型参数和一个可选约束组成。类型参数只是占位符类型的名称（例如 `T`、`U`、`V`、`Key`、`Value` 等）。你可以访问类型、函数或初始值设定项声明的其余部分中的类型参数（及其任何关联类型），包括函数或初始值设定项的签名中的类型参数。
...
你还可以将类型形参替换为本身是泛型类型的特定版本的类型实参（前提是它满足适当的约束和要求）。例如，您可以将 `Array<Element>` 中的类型形参 `Element` 替换为数组的特定版本 `Array<Int>`，以形成一个数组，其元素本身就是数组整数。
