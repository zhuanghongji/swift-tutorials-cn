...
将以下划线开头的标识符、第一个参数标签以下划线开头的下标和第一个参数标签以下划线开头的初始化器视为内部，即使它们的声明具有访问级别修饰符 public。此约定允许框架作者标记客户端不得与之交互或依赖的 API 的一部分，即使某些限制要求声明是公开的。此外，以两个下划线开头的标识符是为 Swift 编译器和标准库保留的。
