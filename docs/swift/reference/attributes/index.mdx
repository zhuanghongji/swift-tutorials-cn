...
Swift 为属性包装器的初始化提供了两种形式的语法糖。你可以在包装值的定义中使用赋值语法，将赋值右侧的表达式作为参数传递给属性包装器的初始化器的参数。你还可以在将属性应用于属性时向属性提供参数，这些参数将传递给属性包装器的初始化程序。例如，在下面的代码中，调用定义的每个初始化器。wrappedValueSomeStructSomeWrapper
