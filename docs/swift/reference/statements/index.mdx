...
<OriginalLink
  title="组表达式并控制执行流程"
  value="https://docs.swift.org/swift-book/documentation/the-swift-programming-language/statements"
/>
...
评估语句的控制表达式，然后将其与每种情况下指定的模式进行比较。switch 如果找到匹配项，程序将执行该案例范围内列出的语句。每个案例的范围不能为空。因此，您必须在:每个 case 标签的冒号 ( ) 后至少包含一个语句。break 如果您不打算执行匹配案例正文中的任何代码，请使用单个语句。

您的代码可以分支的表达式的值非常灵活。例如，除了整数和字符等标量类型的值外，您的代码还可以对任何类型的值进行分支，包括浮点数、字符串、元组、自定义类的实例和可选值。控制表达式的值甚至可以与枚举中的案例值相匹配，并检查是否包含在指定的值范围内。有关如何在 switch 语句中使用这些不同类型的值的示例，请参阅控制流中的 Switch。
...
语句 switch 还可以包括由关键字引入的默认情况 default。仅当没有其他情况与控制表达式匹配时，默认情况下的代码才会执行 ​​。一个 switch 语句只能包含一个默认情况，它必须出现在语句的末尾 switch。
...
在 Swift 中，控制表达式类型的每个可能值都必须匹配至少一个 case 模式的值。当这根本不可行时（例如，当控制表达式的类型为 时 Int），您可以包含一个默认情况来满足要求。

## 切换未来的枚举案例

非冻结枚举是一种特殊的枚举，将来可能会获得新的枚举案例——即使在您编译并发布应用程序之后。切换非冻结枚举需要额外考虑。当库的作者将枚举标记为非冻结时，他们保留添加新枚举案例的权利，并且与该枚举交互的任何代码都必须能够处理这些未来案例而无需重新编译。在库进化模式下编译的代码、标准库中的代码、Apple 框架的 Swift 覆盖以及 C 和 Objective-C 代码都可以声明非冻结枚举。有关冻结和非冻结枚举的信息，请参阅 frozen。

切换非冻结枚举值时，始终需要包含默认情况，即使枚举的每个情况都已经有相应的切换情况。您可以将该@unknown 属性应用于默认情况，这表明默认情况应该只匹配将来添加的枚举情况。如果默认情况与编译器时已知的任何枚举情况相匹配，Swift 会发出警告。此未来警告通知您库作者向枚举添加了一个新案例，但没有相应的切换案例。

以下示例切换标准库枚举的所有三种现有情况。如果您将来添加其他案例，编译器会生成一条警告，指示您需要更新 switch 语句以将新案例考虑在内。Mirror.AncestorRepresentation
...
匹配案例中的代码执行完毕后，程序从该 switch 语句中退出。程序执行不会继续或“失败”到下一个案例或默认案例。也就是说，如果您希望从一个案例继续执行到下一个案例，请在您希望继续执行的案例中显式包含一个 fallthrough 仅由关键字组成的语句。fallthrough 有关该 fallthrough 声明的更多信息，请参阅下面的 Fallthrough 声明。
...
您可以为循环语句、if 语句 switch、语句或 do 带有语句标签的语句添加前缀，该语句由标签的名称和紧跟的冒号 (:) 组成。将语句标签与 breakandcontinue 语句一起使用，以明确说明您希望如何更改循环语句或语句中的控制流 switch，如下面的 Break Statement 和 Continue Statement 中所述。

标记语句的范围是语句标签后面的整个语句。您可以嵌套标签语句，但每个语句标签的名称必须是唯一的。
...
当 break 语句后面没有语句标签的名称时，它会结束该 switch 语句或出现该语句的最内层封闭循环语句的程序执行。您不能使用未标记的 break 语句来中断 if 语句。
...
如果多个 defer 语句出现在同一范围内，则它们出现的顺序与它们执行的顺序相反。首先在给定范围内执行最后一条 defer 语句意味着最后一条 defer 语句中的语句可以引用将被其他语句清理的资源 defer。
...
swift()和平台条件的版本号 compiler()由主版本号、可选的次版本号、可选的补丁号等组成，.版本号的每个部分之间用点 ( ) 分隔。比较运算符和版本号之间不能有空格。的版本 compiler()是编译器版本，与传递给编译器的 Swift 版本设置无关。的版本 swift()是当前正在编译的语言版本。例如，如果您在 Swift 4.2 模式下使用 Swift 5 编译器编译代码，则编译器版本为 5，语言版本为 4.2。使用这些设置，以下代码打印所有三个消息：
...
您可以使用逻辑运算符&&、||和 组合和取反编译条件，!并使用括号进行分组。这些运算符与用于组合普通布尔表达式的逻辑运算符具有相同的结合性和优先级。

类似于 if 语句，可以添加多个条件分支来测试不同的编译条件。您可以使用子句添加任意数量的附加分支#elseif。您还可以使用子句添加最终的附加分支#else。包含多个分支的条件编译块具有以下形式：
...
第一种形式将错误消息作为致命错误发出并终止编译过程。第二种形式发出警告消息作为非致命警告并允许编译继续进行。您将诊断消息写为静态字符串文字。静态字符串文字不能使用字符串插值或连接等功能，但它们可以使用多行字符串文字语法。
...
您使用可用性条件来执行代码块，具体取决于您要使用的 API 在运行时是否可用。当编译器验证该代码块中的 API 是否可用时，它会使用来自可用性条件的信息。

可用性条件采用以逗号分隔的平台名称和版本列表。使用、、和作为平台名称，并包括相应的版本号。该参数是必需的，它指定在任何其他平台上，由可用性条件保护的代码块的主体在您的目标指定的最小部署目标上执行。iOSmacOSwatchOStvOS\*

与布尔条件不同，您不能使用&&和等逻辑运算符组合可用性条件||。不使用 using!来否定可用性条件，而是使用不可用性条件，其具有以下形式：
