...
<OriginalLink
  title="访问、修改和分配值"
  value="https://docs.swift.org/swift-book/documentation/the-swift-programming-language/expressions"
/>

在 Swift 中，有四种表达式：前缀表达式、中缀表达式、主表达式和后缀表达式。计算一个表达式返回一个值，导致副作用，或两者兼而有之。

前缀和中缀表达式使您可以将运算符应用于较小的表达式。基本表达式在概念上是最简单的一种表达式，它们提供了一种访问值的方法。后缀表达式，如前缀和中缀表达式，让您可以使用后缀构建更复杂的表达式，例如函数调用和成员访问。下面的部分详细描述了每种表达式。
...
当中缀运算符左侧的表达式标有 `try`、`try?` 或 时 `try!`，该运算符适用于整个中缀表达式。也就是说，您可以使用括号来明确说明运算符的应用范围。
...
标有 的表达式 await 称为潜在悬点。异步函数的执行可以在每个标有 的表达式处暂停 await。此外，并发代码的执行永远不会在任何其他点暂停。这意味着潜在挂起点之间的代码可以安全地更新需要暂时打破不变量的状态，前提是它在下一个潜在挂起点之前完成更新。

表达式 await 只能出现在异步上下文中，例如传递给函数的尾随闭包。它不能出现在语句体中，也不能出现在同步函数类型的自动关闭中。async(priority:operation:)defer

当中缀运算符左侧的表达式用运算符标记时 await，该运算符适用于整个中缀表达式。也就是说，您可以使用括号来明确说明运算符的应用范围。
...
运算 as 符在编译时知道转换总是成功时执行转换，例如向上转换或桥接。向上转型使您可以将表达式用作其类型超类型的实例，而无需使用中间变量。以下方法是等效的：
...
桥接让您可以使用 Swift 标准库类型的表达式，例如 String 它对应的 Foundation 类型，而 NSString 无需创建新实例。有关桥接的更多信息，请参阅使用基础类型。
...
初级表达式是最基本的一种表达式。它们可以单独用作表达式，也可以与其他标记组合以构成前缀表达式、中缀表达式和后缀表达式。
...
表达式的字符串值的格式为 module / file，其中 file 是出现表达式的文件的名称，而 module 是该文件所属的模块的名称。表达式的字符串值是出现该表达式的文件的完整文件系统路径。如行控制语句中所述，这两个值都可以通过 更改。因为没有嵌入源文件的完整路径，与 不同，它为您提供了更好的隐私并减少了编译二进制文件的大小。避免使用测试之外的内容、构建脚本或其他未成为发布程序一部分的代码。#fileID#filePath#sourceLocation#fileID#filePath#filePath
...
字典中的最后一个表达式后面可以跟一个可选的逗号。字典文字的值具有 type [Key: Value]，其中 Key 是其键表达式的类型，Value 是其值表达式的类型。如果存在多种类型的表达式，Key 并且 Value 是它们各自值最接近的公共超类型。空字典字面量写成一对括号 ( [:]) 内的冒号，以区别于空数组字面量。您可以使用空字典文字来创建指定键和值类型的空字典文字。
...
在初始化程序、下标或实例方法中，self 指代它出现的类型的当前实例。在类型方法中，self 指代它出现的当前类型。
...
闭包表达式创建一个闭包，在其他编程语言中也称为 lambda 或匿名函数。与函数声明一样，闭包包含语句，并从其封闭范围中捕获常量和变量。它具有以下形式：
...
闭包表达式可以在不存储在变量或常量中的情况下使用，例如当您立即将闭包用作函数调用的一部分时。上面代码中传递给的闭包表达式就是这种直接使用的例子。因此，闭包表达式是转义还是非转义取决于表达式周围的上下文。如果立即调用或作为非转义函数参数传递，则闭包表达式是非转义的。否则，闭包表达式正在转义。myFunction
...
weak 如果表达式值的类型是一个类，您可以在捕获列表中用或标记表达式 unowned 以捕获对表达式值的弱引用或无主引用。
...
您还可以将任意表达式绑定到捕获列表中的命名值。创建闭包时会计算表达式，并使用指定的强度捕获值。例如：
...
隐式成员表达式后面可以跟一个后缀运算符或后缀表达式中列出的其他后缀语法。这称为链式隐式成员表达式。尽管所有链式后缀表达式都具有相同的类型是很常见的，但唯一的要求是整个链式隐式成员表达式需要可转换为其上下文隐含的类型。具体来说，如果隐含类型是可选类型，则可以使用非可选类型的值，如果隐含类型是类类型，则可以使用其子类之一的值。例如：
...
括号表达式由括号包围的表达式组成。您可以使用括号通过显式分组表达式来指定操作的优先级。分组括号不会改变表达式的类型——例如，的类型(1)只是 Int.
...
:::note
空元组表达式和空元组类型都是()用 Swift 编写的。因为 Void 是 的类型别名()，所以您可以使用它来编写空元组类型。但是，与所有类型别名一样，Void 始终是一种类型——您不能用它来编写空元组表达式。
:::
...
键路径表达式指的是类型的属性或下标。您在动态编程任务中使用键路径表达式，例如键值观察。它们具有以下形式：
...
路径可参考创建 self 身份密钥路径（ `\.self` ）。身份密钥路径指的是整个实例，因此您可以使用它一步访问和更改存储在变量中的所有数据。例如：
...
您可以混合和匹配键路径的组件以访问深度嵌套在类型中的值。以下代码通过使用组合这些组件的键路径表达式来访问数组字典的不同值和属性。
...
您可以在通常提供函数或闭包的上下文中使用键路径表达式。具体来说，您可以使用一个关键路径表达式，其根类型为且其路径产生一个 type 的值，而不是一个 type 的函数或闭包。`SomeTypeValue(SomeType) -> Value`
...
关键路径表达式的任何副作用仅在计算表达式的位置计算。例如，如果您在关键路径表达式的下标内进行函数调用，则该函数仅在计算表达式时被调用一次，而不是每次使用关键路径时都被调用。
...
键路径字符串表达式允许您访问用于引用 Objective-C 中的属性的字符串，用于键值编码和键值观察 API。它具有以下形式：
...
后缀表达式是通过将后缀运算符或其他后缀语法应用于表达式而形成的。从句法上讲，每个主表达式也是一个后缀表达式。
...
由这些隐式转换创建的指针仅在函数调用期间有效。为避免未定义的行为，请确保您的代码永远不会在函数调用结束后保留 ​​ 指针。

:::note
将数组隐式转换为不安全指针时，Swift 通过根据需要转换或复制数组来确保数组的存储是连续的。Array 例如，您可以将此语法用于从一个子类桥接到的数组，NSArray 该子类没有就其存储制定 API 合同。如果您需要保证数组的存储已经是连续的，那么隐式转换永远不需要完成这项工作，请使用 instead of 。ContiguousArrayArray
:::

使用&instead of an explicit function like 可以帮助使对低级 C 函数的调用更具可读性，尤其是当函数采用多个指针参数时。但是，当从其他 Swift 代码调用函数时，避免显式使用而不是使用不安全的 API。withUnsafePointer(to:)&
...
在函数调用表达式中使用初始化表达式来初始化一个类型的新实例。您还可以使用初始化表达式来委托给超类的初始化器。
...
如果按名称指定类型，则无需使用初始化表达式即可访问该类型的初始化器。在所有其他情况下，您必须使用初始化表达式。
...
您可以将这种多行链接语法与编译器控制语句结合起来，以控制何时调用每个方法。例如，以下代码在 iOS 上使用了不同的过滤规则：
...
#if 在、和其他编译指令之间#endif，条件编译块可以包含一个隐式成员表达式，后跟零个或多个后缀，以形成后缀表达式。它还可以包含另一个条件编译块，或这些表达式和块的组合。

您可以在任何可以编写显式成员表达式的地方使用此语法，而不仅仅是在顶级代码中。

在条件编译块中，编译指令的分支#if 必须包含至少一个表达式。其他分支可以是空的。
...
第二种形式的计算结果为 type 的值。使用此形式将类型作为值访问。例如，因为评估为类型本身，您可以将其传递给接受类型级参数的函数或方法。SomeClass.selfSomeClass
...
强制值表达式解包一个您确定不是的可选值 nil。它具有以下形式：
...
可选链表达式必须出现在后缀表达式中，它们会导致以特殊方式计算后缀表达式。如果可选链接表达式的值为 nil，则忽略后缀表达式中的所有其他操作，并且整个后缀表达式的计算结果为 nil。如果可选链接表达式的值不是 nil，则可选链接表达式的值被解包并用于计算后缀表达式的其余部分。在任何一种情况下，后缀表达式的值仍然是可选类型。

如果包含可选链接表达式的后缀表达式嵌套在其他后缀表达式中，则只有最外层的表达式返回可选类型。在下面的例子中，当 c 不是时 nil，它的值被解包并用于评估.property，其值用于评估。整个表达式有一个可选类型的值。.performAction()c?.property.performAction()
