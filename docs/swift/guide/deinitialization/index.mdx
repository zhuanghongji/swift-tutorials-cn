...
在释放类实例之前立即调用析构器。您使用 deinit 关键字编写反初始化器，类似于使用 init 关键字编写初始化器的方式。析构器仅适用于类类型。

## 取消初始化是如何工作的

Swift 会在不再需要时自动释放您的实例，以释放资源。Swift 通过自动引用计数( ARC )处理实例的内存管理，如自动引用计数中所述。通常，您不需要在释放实例时执行手动清理。但是，当您使用自己的资源时，您可能需要自己执行一些额外的清理工作。例如，如果您创建一个自定义类来打开一个文件并向其中写入一些数据，您可能需要在释放该类实例之前关闭该文件。
...
就在实例释放发生之前，析构器被自动调用。不允许您自己调用析构器。超类析构器由它们的子类继承，超类析构器在子类析构器实现结束时自动调用。超类的析构器总是被调用，即使子类不提供它自己的析构器。
...
玩家现在已经离开了游戏。这通过将可选变量设置为 来表示，意思是“无实例”。在发生这种情况时，变量对实例的引用被破坏了。没有其他属性或变量仍然引用该实例，因此它被释放以释放其内存。就在这发生之前，它的去初始化器被自动调用，它的硬币被返还给银行。`playerOnenilPlayerplayerOnePlayerPlayer`
