...
Swift 的 Array 类型还提供了一个初始化器，用于创建一个特定大小的数组，并将其所有值设置为相同的默认值。你向此初始值设定项传递适当类型的默认值（称为 repeating）：以及该值在新数组中重复的次数（称为 count）：
...
同样，你可以使用 方法从数组中删除一个项目 remove(at:)。此方法删除指定索引处的项目并返回删除的项目（尽管如果不需要，可以忽略返回值）：
