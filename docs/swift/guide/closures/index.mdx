...
当你调用此函数加载图片时，你提供了两个闭包。第一个闭包是一个完成处理程序，它在成功下载后显示图片。第二个闭包是向用户显示错误的错误处理程序。
...
在此示例中，该函数将其网络任务分派到后台，并在网络任务完成时调用两个完成处理程序之一。以这种方式编写函数可以让你清楚地将负责处理网络故障的代码与在成功下载后更新用户界面的代码分开，而不是只使用一个闭包来处理这两种情况。loadPicture(from:completion:onFailure:)
