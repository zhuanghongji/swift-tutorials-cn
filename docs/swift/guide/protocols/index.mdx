...
<OriginalLink
  title="定义符合类型必须实现的要求"
  value="https://docs.swift.org/swift-book/documentation/the-swift-programming-language/protocols"
/>

协议定义了适合特定任务或功能的方法、属性和其他要求的蓝图。然后该协议可以被类、结构或枚举采用，以提供这些要求的实际实现。任何满足协议要求的类型都被称为符合该协议。
...
如果协议要求属性是可获取和可设置的，则常量存储属性或只读计算属性无法满足该属性要求。如果协议只要求一个属性是可获取的，那么任何类型的属性都可以满足该要求，并且如果这对您自己的代码有用，那么属性也是可设置的也是有效的。
...
该协议需要一个一致的类型来提供一个完全限定的名称。该协议没有指定任何关于符合类型的性质的其他内容——它只指定该类型必须能够为自己提供一个完整的名称。该协议规定任何类型都必须有一个名为 的可获取实例属性，该属性的类型为。FullyNamedFullyNamedfullNameString
...
此示例定义了一个名为 的结构 Person，它表示一个特定的命名人。它声明它采用该协议作为其定义第一行的一部分。FullyNamed
...
与类型属性要求一样，static 当在协议中定义它们时，您总是在类型方法要求前面加上关键字。即使类型方法要求在由类实现时以 classor 关键字为前缀，也是如此：static
...
方法有时需要修改（或改变）它所属的实例。对于值类型（即结构和枚举）上的实例方法，您将关键字放在 mutating 方法的 func 关键字之前，以指示允许该方法修改它所属的实例以及该实例的任何属性。此过程在从实例方法中修改值类型中进行了描述。

如果您定义了一个协议实例方法要求，旨在改变采用该协议的任何类型的实例，请使用关键字标记该方法 mutating 作为协议定义的一部分。这使结构和枚举能够采用协议并满足该方法要求。
...
如果您 Togglable 为结构或枚举实现协议，则该结构或枚举可以通过提供 toggle()也标记为 mutating.
...
协议可能需要特定的初始化程序由符合类型来实现。您将这些初始化器作为协议定义的一部分，以与普通初始化器完全相同的方式编写，但没有大括号或初始化器主体：
...
您可以将协议初始化器要求作为指定初始化器或便利初始化器实现在符合类上。在这两种情况下，您都必须使用修饰符标记初始化程序实现 required：
...
使用修饰符 required 可确保您为符合类的所有子类提供初始化要求的显式或继承实现，以便它们也符合协议。

有关必需的初始化器的更多信息，请参阅必需的初始化器。

:::note
您不需要 required 在用修饰符标记的类上用修饰符标记协议初始化器实现 final，因为最终类不能子类化。有关 final 修饰符的更多信息，请参阅防止覆盖。
:::

如果一个子类重写了一个超类的指定初始化器，并且还实现了一个协议的匹配初始化器要求，则用 和 required 修饰符标记初始化器实现 override：
...
协议本身实际上并不实现任何功能。尽管如此，您可以在代码中将协议用作完全成熟的类型。将协议用作类型有时称为存在类型，它来自短语“存在类型 T，使得 T 符合协议”。
...
:::note
因为协议是类型，所以它们的名称以大写字母开头（例如 and ）以匹配 Swift 中其他类型的名称（例如,和）。FullyNamedRandomNumberGeneratorIntStringDouble
:::
...
此示例定义了一个名为 的新类 Dice，它表示用于棋盘游戏的 n 面骰子。Dice 实例有一个名为 的整数属性 sides，表示它们有多少面，还有一个名为 的属性 generator，它提供一个随机数生成器，从中创建骰子掷骰值。

该 generator 属性的类型为。因此，您可以将其设置为采用该协议的任何类型的实例。您分配给该属性的实例不需要其他任何东西，除了该实例必须采用该协议。因为它的类型是，所以类内部的代码只能以适用于所有符合该协议的生成器的方式进行交互。这意味着它不能使用由生成器的底层类型定义的任何方法或属性。但是，您可以从协议类型向下转换为基础类型，就像从超类向下转换为子类一样，如向下转换中所述。RandomNumberGeneratorRandomNumberGeneratorRandomNumberGeneratorRandomNumberGeneratorDicegenerator

Dice 还有一个初始化器，用于设置其初始状态。这个初始化器有一个名为 的参数 generator，它的类型也是。初始化新实例时，您可以将任何符合类型的值传递给此参数。RandomNumberGeneratorDice
...
Snakes and Ladders 游戏板设置发生在类的 init()初始化程序中。所有游戏逻辑都被转移到协议的 play 方法中，该方法使用协议的必需 dice 属性来提供其掷骰值。

请注意，该 delegate 属性被定义为可选的 ，因为玩游戏不需要委托。因为它是可选类型，所以该属性会自动设置为初始值。此后，游戏实例化器可以选择将属性设置为合适的委托。因为该协议是仅限类的，所以您可以将委托声明为以防止引用循环。DiceGameDelegatedelegatenilDiceGameDelegateweak
...
上面显示的实现使用参数来打印有关即将玩的游戏的一些介绍信息。该参数的类型为, not ，因此只能访问和使用作为协议的一部分实现的方法和属性。但是，该方法仍然能够使用类型转换来查询底层实例的类型。在此示例中，它检查是否实际上是幕后的实例，如果是，则打印一条适当的消息。gameDidStart(_:)gamegameDiceGameSnakesAndLaddersgameDidStart(_:)DiceGamegameSnakesAndLadders
...
您可以扩展现有类型以采用并符合新协议，即使您无权访问现有类型的源代码。扩展可以向现有类型添加新的属性、方法和下标，因此能够添加协议可能要求的任何要求。有关扩展的更多信息，请参阅扩展。
...
泛型可能只有在某些条件下才能满足协议的要求，例如当类型的泛型参数符合协议时。您可以通过在扩展类型时列出约束来使泛型类型有条件地符合协议。通过编写通用子句，在您采用的协议名称之后写入这些约束 where。有关通用 where 子句的更多信息，请参阅通用 Where 子句。
...
Equatable 在许多简单的情况下，Swift 可以自动为、Hashable 和提供协议一致性 Comparable。使用这种综合实现意味着您不必编写重复的样板代码来自己实现协议要求。
...
协议可以继承一个或多个其他协议，并可以在它继承的要求之上添加更多要求。协议继承的语法类似于类继承的语法，但可以选择列出多个继承的协议，以逗号分隔：
...
此示例定义了一个新协议，它继承自。任何采用的东西都必须满足由 强制执行的所有要求，以及由 强制执行的附加要求。在此示例中，添加了一个要求以提供一个名为的可获取属性，该属性返回一个.PrettyTextRepresentableTextRepresentablePrettyTextRepresentableTextRepresentablePrettyTextRepresentablePrettyTextRepresentableprettyTextualDescriptionString
...
您可以通过将协议添加到协议的继承列表来将协议采用限制为类类型（而不是结构或枚举） 。AnyObject
...
要求一个类型同时符合多个协议可能很有用。您可以使用协议组合将多个协议组合成单个需求。协议组合的行为就像您定义了一个临时本地协议，该协议具有组合中所有协议的组合要求。协议组合不定义任何新的协议类型。

协议组合的形式为。您可以根据需要列出任意数量的协议，并使用与号 ( ) 将它们分隔开。除了它的协议列表之外，协议组合还可以包含一个类类型，您可以使用它来指定所需的超类。SomeProtocol & AnotherProtocol&
...
在此示例中，协议对名为 的可获取属性 Named 有一个单一要求。该协议对名为 的可获取属性有一个单一要求。这两种协议都被称为.StringnameAgedIntagePerson
...
传递给函数无效，因为不是 的子类。同样，如果您创建了不符合协议的子类，则使用该类型的实例进行调用也是无效的。birthdayPersonbeginConcert(in:)PersonLocationLocationNamedbeginConcert(in:)

## 检查协议一致性

您可以使用类型转换中描述的 isand 运算符来检查协议一致性，并转换为特定协议。检查并转换为协议遵循与检查类型并转换为类型完全相同的语法：as

- 如果实例符合协议，则运算符返回，is 否则返回。truefalse
- 向下转换运算符的版本 as?返回协议类型的可选值，nil 如果实例不符合该协议，则该值是。
- 向下转换运算符的版本 as!强制向下转换为协议类型，如果向下转换不成功则触发运行时错误。

此示例定义了一个名为 的协议，具有一个名为 的可获取属性的单一属性要求：HasAreaDoublearea
...
您可以定义协议的可选要求。这些要求不必由符合协议的类型来实现。optional 作为协议定义的一部分，可选要求以修饰符为前缀。可选要求可用，以便您可以编写与 Objective-C 互操作的代码。协议和可选要求都必须标有该@objc 属性。请注意，@objc 协议只能被类采用，而不能被结构或枚举采用。

当您在可选需求中使用方法或属性时，其类型自动变为可选。例如，类型的方法 `(Int) -> String` 变为 `((Int) -> String)?` . 请注意，整个函数类型都包含在可选值中，而不是方法的返回值。

可以使用可选链接调用可选协议要求，以说明该要求未由符合协议的类型实现的可能性。您可以通过在调用方法名称后写一个问号来检查可选方法的实现，例如. 有关可选链接的信息，请参阅可选链接。someOptionalMethod?(someArgument)
...
:::note
严格来说，您可以编写符合协议要求的自定义类，而无需实现任一协议要求。毕竟，它们都是可选的。尽管在技术上是允许的，但这并不是一个很好的数据源。CounterDataSource
:::
...
您可以使用 的实例作为新实例的数据源：ThreeSourceCounter
...
您可以将 的实例与现有实例一起使用以从零开始计数。一旦计数器达到零，就不再计数：TowardsZeroSourceCounter-4
...
可以扩展协议以提供符合类型的方法、初始值设定项、下标和计算属性实现。这允许您在协议本身上定义行为，而不是在每种类型的单独一致性或全局函数中。
...
定义协议扩展时，可以指定在扩展的方法和属性可用之前，符合类型必须满足的约束。您可以在通过编写通用子句扩展的协议名称之后编写这些约束 where。有关通用 where 子句的更多信息，请参阅通用 Where 子句。

Collection 例如，您可以定义适用于任何其元素符合协议的集合的协议扩展 Equatable。通过将集合的元素约束到 Equatable 协议（标准库的一部分），您可以使用==and!=运算符来检查两个元素之间的相等性和不相等性。
