...
协议可能需要特定的初始化程序由符合类型来实现。你将这些初始化器作为协议定义的一部分，以与普通初始化器完全相同的方式编写，但没有大括号或初始化器主体：
...
你可以将协议初始化器要求作为指定初始化器或便利初始化器实现在符合类上。在这两种情况下，你都必须使用修饰符标记初始化程序实现 required：
...
使用修饰符 required 可确保你为符合类的所有子类提供初始化要求的显式或继承实现，以便它们也符合协议。

有关必需的初始化器的更多信息，请参阅必需的初始化器。

:::note
你不需要 required 在用修饰符标记的类上用修饰符标记协议初始化器实现 final，因为最终类不能子类化。有关 final 修饰符的更多信息，请参阅防止覆盖。
:::
...
你可以定义协议的可选要求。这些要求不必由符合协议的类型来实现。optional 作为协议定义的一部分，可选要求以修饰符为前缀。可选要求可用，以便你可以编写与 Objective-C 互操作的代码。协议和可选要求都必须标有该@objc 属性。请注意，@objc 协议只能被类采用，而不能被结构或枚举采用。
...
可以扩展协议以提供符合类型的方法、初始值设定项、下标和计算属性实现。这允许你在协议本身上定义行为，而不是在每种类型的单独一致性或全局函数中。
