...
Swift 提供了所有 C 和 Objective-C 基本类型的自己的版本，包括 `Int` 整数、`Double` 和 `Float` 浮点数、`Bool` 布尔值以及 `String` 文本数据。Swift 还提供了三种主要集合类型的强大版本，`Array`、`Set` 和 `Dictionary`，如 [集合类型](../collection-types/) 中所述。
...
与 C 中的多行注释不同，Swift 中的多行注释可以嵌套在其他多行注释中。你可以通过启动一个多行注释块然后在第一个块中启动第二个多行注释来编写嵌套注释。然后关闭第二个块，然后关闭第一个块：
...
与许多其他语言不同，Swift 不要求你在代码中的每个语句后都写一个分号 ( ;)，尽管你可以根据需要这样做。但是，如果你想在一行中编写多个单独的语句，则需要分号：
...
SomeType(ofInitialValue)是调用 Swift 类型的初始值设定项并传入初始值的默认方式。在幕后，UInt16 有一个接受 UInt8 值的初始化器，所以这个初始化器用于 UInt16 从现有的 UInt8. 但是，你不能在此处传入任何类型——它必须是提供初始值设定项的类型 UInt16。扩展现有类型以提供接受新类型（包括你自己的类型定义）的初始值设定项包含在 Extensions 中。
...
和的类型是根据它们是用布尔文字值初始化的事实推断出来的。与上面一样，你不需要声明常量或变量，就像你将它们设置为或创建它们一样。当使用已知类型的其他值初始化常量或变量时，类型推断有助于使 Swift 代码更加简洁和可读。orangesAreOrangeturnipsAreDeliciousBoolIntDoubleBooltruefalse
...
你可以根据类型的任意排列创建元组，并且它们可以包含任意数量的不同类型。没有什么能阻止你拥有类型为(Int, Int, Int)、 或 的元组(String, Bool)，或者实际上是你需要的任何其他排列。
...
你可以通过调用 Swift 标准库中的函数来编写断言。你向此函数传递一个计算结果为 or 的表达式，以及在条件结果为 时显示的消息。例如：assert(_:_:file:line:)truefalsefalse
...
你通过调用函数编写前提条件。你向此函数传递一个计算结果为 or 的表达式，以及在条件结果为 时显示的消息。例如：precondition(_:_:file:line:)truefalsefalse
...
你还可以调用该函数来指示发生了故障——例如，如果采用了开关的默认情况，但所有有效输入数据都应该由开关的其他情况之一处理。preconditionFailure(\_:file:line:)
