...
通常创建枚举以支持特定类或结构的功能。同样，定义实用类和结构纯粹是为了在更复杂类型的上下文中使用会很方便。为实现这一点，Swift 使您能够定义嵌套类型，从而将支持的枚举、类和结构嵌套在它们所支持的类型的定义中。
...
因为是一个没有自定义初始值设定项的结构，所以它有一个隐式成员初始值设定项，如结构类型的成员初始值设定项中所述。您可以使用此初始化程序来初始化一个名为的新常量：BlackjackCardtheAceOfSpades
