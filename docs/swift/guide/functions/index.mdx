...
Swift 中的每个函数都有一个类型，由函数的参数类型和返回类型组成。你可以像 Swift 中的任何其它类型一样使用此类型，这使得将函数作为参数传递给其它函数以及从函数返回函数变得容易。函数也可以写在其它函数中，以将有用的功能封装在嵌套函数范围内。
...
你可以通过向函数传递一个标记的参数值和一个标记在括号中的参数值来调用该函数，并用逗号分隔。请注意，此函数与前面部分中显示的函数不同。虽然这两个函数的名称都以 开头，但函数有两个参数，而函数只有一个。`greet(person:alreadyGreeted:)StringpersonBoolalreadyGreetedgreet(person:)greetgreet(person:alreadyGreeted:)greet(person:)`
...
此示例定义了一个名为 的函数，它具有三个参数。第一个参数称为，类型为。你可以传递该类型的任何函数作为第一个参数的参数。第二个和第三个参数称为 and ，并且都是 type 。这些用作提供的数学函数的两个输入值。printMathResult(_:_:\_:)mathFunction(Int, Int) -> IntabInt
