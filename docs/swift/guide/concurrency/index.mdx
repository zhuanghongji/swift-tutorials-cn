...
在此示例中，所有三个调用都在不等待前一个调用完成的情况下开始。如果有足够的可用系统资源，它们可以同时运行。这些函数调用都没有标有 ，因为代码不会暂停以等待函数的结果。相反，执行会一直持续到定义的那一行——此时，程序需要这些异步调用的结果，因此你写入暂停执行，直到所有三张照片都完成下载。`downloadPhoto(named:)awaitphotosawait`
...
除了前面部分描述的结构化并发方法外，Swift 还支持非结构化并发。与属于任务组的任务不同，非结构化任务没有父任务。你可以完全灵活地以你的程序需要的任何方式管理非结构化任务，但你也对它们的正确性负全部责任。要创建在当前 actor 上运行的非结构化任务，请调用初始化程序。要创建不属于当前参与者的非结构化任务（更具体地称为分离任务），请调用类方法。这两个操作都会返回一个你可以与之交互的任务——例如，等待其结果或取消它。Task.init(priority:operation:)Task.detached(priority:operation:)
...
该 `update(with:)` 方法已经在 actor 上运行，因此它不会像 maxwith 那样标记它对属性的访问 await。此方法还显示了 actor 一次只允许一个任务与其可变状态交互的原因之一：对 actor 状态的某些更新会暂时破坏不变量。Actor 跟踪温度列表和最高温度，并在你记录新测量值时更新最高温度。在更新过程中，在添加新测量值之后但在更新之前，温度记录器处于临时不一致状态。防止多个任务同时与同一个实例交互可以防止出现类似以下事件序列的问题：TemperatureLoggermax
