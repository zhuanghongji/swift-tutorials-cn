...
此示例创建一个名为 `productBarcode` 的新变量，并为其分配一个关联元组值 `(8, 85909, 51226, 3)` 的 `Barcode.upc` 值。
...
此示例使用可选绑定（optional binding）来尝试访问原始值为 `11` 的行星。语句 `if let somePlanet = Planet(rawValue: 11)` 创建一个可选的 `Planet`，并将 `somePlanet` 设置为该可选 `Planet` 的值（如果可以检索）。在这种情况下，不可能检索位置为 `11` 的行星，因此会执行 `else` 分支。
