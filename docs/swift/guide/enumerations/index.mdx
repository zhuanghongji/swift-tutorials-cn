...
如果你使用原始值类型定义枚举，则该枚举会自动接收一个初始化器，该初始化器采用原始值类型的值（作为称为 的参数）并返回枚举 case 或。你可以使用此初始化器来尝试创建枚举的新实例。rawValuenil
...
:::note
原始值初始化器是一个可失败初始化器，因为并非每个原始值都会返回一个枚举案例。有关详细信息，请参阅可失败的初始化器。
:::
