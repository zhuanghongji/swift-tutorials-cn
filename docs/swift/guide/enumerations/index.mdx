...
如果你使用原始值类型定义枚举，则该枚举会自动接收一个初始化器，该初始化器采用原始值类型的值（作为称为 的参数）并返回枚举 case 或。你可以使用此初始化程序来尝试创建枚举的新实例。rawValuenil
...
然而，并非所有可能的 Int 值都能找到匹配的行星。因此，原始值初始化器总是返回一个可选的枚举案例。在上面的示例中，是类型，或“可选”。possiblePlanetPlanet?Planet

:::note
原始值初始化器是一个可失败初始化器，因为并非每个原始值都会返回一个枚举案例。有关详细信息，请参阅可失败的初始化程序。
:::

如果你尝试查找位置为 的行星 11，则原始值初始化器返回的可选 Planet 值将为 nil：
