...
如果你使用可选链接对可选值调用此方法，则该方法的返回类型将为 Void?，而不是 Void，因为通过可选链接调用时，返回值始终为可选类型。这使你能够使用 if 语句来检查是否可以调用该方法，即使该方法本身没有定义返回值。比较调用的返回值以查看方法调用是否成功：printNumberOfRooms()printNumberOfRoomsnil
