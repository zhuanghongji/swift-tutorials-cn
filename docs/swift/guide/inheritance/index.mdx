...
你可以通过将方法、属性或下标标记为 final 来防止其被覆盖。final 通过在方法、属性或下标的介绍关键字（例如 `final var`、`final func`、`final class func` 和 `final subscript` ）之前编写修饰符来实现。

任何试图覆盖子类中的最终方法、属性或下标的尝试都会被报告为编译时错误。添加到扩展中的类的方法、属性或下标也可以在扩展的定义中标记为最终的。

你可以通过在其类定义 ( ) 中的关键字 `final` 之前编写修饰符来将整个类标记为 `final` 。任何对 final 类进行子类化的尝试都会被报告为编译时错误。classfinal class
