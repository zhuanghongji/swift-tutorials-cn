...
你还可以使用 for-in 带有数字范围的循环。此示例打印五次表中的前几个条目：
...
与 C 不同，Swift 允许多个 switchcase 考虑相同的一个或多个值。事实上，点 (0, 0) 可以匹配此示例中的所有四种情况。但是，如果可能有多个匹配项，则始终使用第一个匹配项。点 (0, 0) 将 case (0, 0)首先匹配，因此所有其它匹配情况将被忽略。
