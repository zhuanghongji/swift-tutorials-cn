...
类型转换是一种检查实例类型的方法，或者将该实例视为与其自身类层次结构中其他地方不同的超类或子类。
...
您可以对类和子类的层次结构使用类型转换来检查特定类实例的类型，并将该实例转换为同一层次结构中的另一个类。下面的三个代码片段定义了一个类的层次结构和一个包含这些类实例的数组，用于类型转换的示例。
...
存储在幕后的 library 静止 Movie 和实例中的项目。Song 但是，如果您遍历此数组的内容，则您收到的项目类型为, 而不是或。为了将它们作为它们的原生类型来使用，您需要检查它们的类型，或者将它们向下转换为不同的类型，如下所述。MediaItemMovieSong
...
某个类类型的常量或变量实际上可能在幕后引用子类的实例。如果您认为是这种情况，您可以尝试使用类型转换运算符（或）向下转换为子类类型。as?as!

因为向下转换可能会失败，所以类型转换运算符有两种不同的形式。条件形式 as?返回您尝试向下转型的类型的可选值。强制形式 as!尝试将向下转换和强制展开结果作为单个复合操作。

as?当您不确定向下转换是否会成功时，请使用类型转换运算符 ( ) 的条件形式。nil 这种形式的运算符将始终返回一个可选值，如果无法进行向下转换，则该值将是。这使您能够检查是否成功进行了向下转换。

as!仅当您确定向下转换总是成功时，才使用类型转换运算符 ( ) 的强制形式。如果您尝试向下转换为不正确的类类型，这种形式的运算符将触发运行时错误。
...
在此示例中，数组中的每个项目可能是一个 Movie，也可能是一个 Song。您事先不知道每个项目使用哪个实际类，因此使用类型转换运算符 ( as?) 的条件形式在每次循环中检查向下转换是合适的：
...
仅当您明确需要它们提供的行为和功能时才使用 Anyand 。最好具体说明您希望在代码中使用的类型。AnyObject
...
要发现已知仅属于 Anyor 类型的常量或变量的特定类型，您可以在语句的 case 中使用 or 模式。下面的示例遍历数组中的项目并使用语句查询每个项目的类型。几个语句的 case 将它们的匹配值绑定到指定类型的常量以使其值能够被打印：AnyObjectisasswitchthingsswitchswitch
