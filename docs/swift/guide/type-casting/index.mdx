...
第一个片段定义了一个名为. 此类为出现在数字媒体库中的任何类型的项目提供基本功能。具体来说，它声明了一个 type 的属性和一个初始化器。（假设所有媒体项目，包括所有电影和歌曲，都有一个名称。）MediaItemnameStringinit(name:)
...
下一个片段定义了 的两个子类。第一个子类封装了有关电影或影片的附加信息。它在基类之上添加了一个属性，并带有相应的初始化器。第二个子类，在基类之上添加了一个属性和初始化器：MediaItemMoviedirectorMediaItemSongartist
...
该示例遍历 library 数组中的所有项目。每次通过时，for-in 循环都会将常量设置为数组中的 item 下一个。MediaItem
...
在该示例中，数组中的每个项目可能是一个 Movie，也可能是一个 Song。你事先不知道每个项目使用哪个实际类，因此使用类型转换运算符 ( as?) 的条件形式在每次循环中检查向下转换是合适的：
...
:::note
类型 Any 表示任何类型的值，包括可选类型。Any 如果你在需要类型值的地方使用可选值，Swift 会给你一个警告。如果确实需要使用可选值作为 Any 值，则可以使用 as 运算符将可选值显式转换为 Any，如下所示。
:::
