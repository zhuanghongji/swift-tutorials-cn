...
你可以为单个类型（类、结构和枚举）以及属于这些类型的属性、方法、初始值设定项和下标分配特定的访问级别。协议可以限制在特定上下文中，全局常量、变量和函数也是如此。
...
类型的访问控制级别也会影响该类型成员（其属性、方法、初始值设定项和下标）的默认访问级别。如果你将类型的访问级别定义为私有或文件私有，则其成员的默认访问级别也将是私有或文件私有。如果你将类型的访问级别定义为内部或公共（或使用默认访问级别内部而不明确指定访问级别），则该类型成员的默认访问级别将为内部。
...
下面的示例定义了一个名为 的全局函数，但没有为该函数本身提供特定的访问级别修饰符。你可能希望此函数具有默认访问级别“内部”，但事实并非如此。事实上，不会像下面写的那样编译：someFunction()someFunction()
...
此外，对于在同一模块中定义的类，你可以覆盖在特定访问上下文中可见的任何类成员（方法、属性、初始值设定项或下标）。对于在另一个模块中定义的类，你可以覆盖任何开放类成员。
...
请注意，如果需要，你可以为 getter 和 setter 分配显式访问级别。下面的示例显示了结构的一个版本，其中结构被定义为具有显式访问级别 public。因此，结构的成员（包括属性）默认具有内部访问级别。你可以通过结合访问级别修饰符和访问级别修饰符，使结构的属性 getter 公开，其属性 setter 私有：TrackedStringnumberOfEditsnumberOfEditspublicprivate(set)
...
默认初始化程序与其初始化的类型具有相同的访问级别，除非该类型定义为 public. 对于定义为 的类型 public，默认初始值设定项被认为是内部的。如果你希望公共类型在另一个模块中使用时可以使用无参数初始化器进行初始化，则必须自己明确提供一个公共无参数初始化器作为类型定义的一部分。
...
或者，你可以使用显式访问级别修饰符（例如，）标记扩展，private 以便为扩展中定义的所有成员设置新的默认访问级别。这个新的默认值仍然可以在单个类型成员的扩展中被覆盖。

如果你使用该扩展来添加协议一致性，则不能为该扩展提供显式访问级别修饰符。相反，协议自己的访问级别用于为扩展中的每个协议要求实现提供默认访问级别。
