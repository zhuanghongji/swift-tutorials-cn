...
缩小具有重叠像素的纹理的解决方案称为 \_mipmapping_mipmap 是一系列纹理，全部包含相同的图像，但分辨率越来越低。原始的全尺寸纹理存在于 mipmap 的基础级别。高于该级别的每个级别逐渐以 2 倍进行下采样。例如，如果你有一个 16 × 16 的纹理，则基本级别也将为 16 × 16。此纹理的完整 mip 链将由大小为 16 的级别组成 × 16（基础）、8 × 8、4 × 4、2 × 2 和 1 × 1，总共五个级别。然后，mipmap 生成器预先计算并存储不同大小和位置的各个区域上纹理的平均值。
