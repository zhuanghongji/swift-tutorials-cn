...
关于 MTLRenderPassAttachmentDescriptor 你需要了解的最后一点是 _load_ 和 \_store actions_MTLLoadAction 是渲染命令编码器的渲染通道开始时执行的操作。这是一个具有三个值的枚举：load、clear 和 dontCareload 保留纹理的现有内容。清除将新值写入附件中的每个像素 dontCare 允许每个像素在渲染过程开始时采用任何值，因为你不关心它是什么。
