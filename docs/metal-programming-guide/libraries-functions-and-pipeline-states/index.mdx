...
<Figure src={require("../figures/021.jpg")} num="7.1" desc="两阶段编译架构" />
...
在构建时，应用程序目标的编译源阶段中存在的任何 .metal 文件都将由 Metal 前端编译器进行编译，并且它们包含的任何函数都将构成默认库。该库可通过默认 MTLDevice 上的方法访问：newDefaultLibrary()。此方法的返回类型是可选的，因此如果在编译时没有构建默认库，它不会使应用程序崩溃。

还可以将 Metal 库二进制文件引入你的项目中。例如，如果你创建了一个开源图像处理框架，则可以将该 Metal 库导入其他人的项目中。库可以作为文件或数据导入。
...
func makeLibrary(filepath: String) 抛出 -> MTLLibrary

此代码的变体允许使用完成处理程序创建异步库：

func makeLibrary（来源：String，选项：MTLCompileOptions？，completionHandler：@escaping MTLNewLibraryCompletionHandler）

**从库加载函数**

成功创建一个库是一个很好的第一步，但如果你不能将这些功能集成到你的应用程序中，那么它不会给你带来任何好处。本节介绍如何将默认库中的函数加载到应用程序中。 MTLLibrary 有一个名为 makeFunction 的方法：

Guard letfragmentProgram = defaultLibrary.makeFunction( name: "lightingFragment") else {return}

guard let vertexProgram = defaultLibrary.makeFunction( name: "lightingVertex") else {return}

此方法是从默认库中提取特定函数的最简单方法。 makeFunction 方法采用一个表示默认库中函数名称的字符串。如果你使用渲染器，则需要提取顶点和片段函数。如果你做计算，你需要拉出一个核函数。
...
- **MTLRenderCommandEncoder** ：该编码器旨在生成单个渲染通道。它访问顶点着色器和片段着色器。 - **MTLBlitCommandEncoder**：该编码器已在第 6 章“ Metal 资源和内存管理”中讨论。它仅用于在缓冲区和纹理之间复制资源。该编码器的工作不需要使用你编写的任何 MTLFunctions。 - **MTLComputeCommandEncoder** ：GPGPU 命令编码器专门访问内核函数。 - **MTLParallelRenderCommandEncoder** ：该编码器采用一个或多个 MTLRenderCommandEncoder 对象。它允许你将每个渲染编码器分配给其自己的线程，以便可以并行执行渲染命令。该编码器不直接访问 MTLLibrary。相反，它是访问 MTLLibrary 的编码器的管理对象。
...
让 pipelineStateDescriptor = MTLRenderPipelineDescriptor() pipelineStateDescriptor.vertexFunction = vertexProgram pipelineStateDescriptor.fragmentFunction = fragmentProgram pipelineStateDescriptor.vertexDescriptor = vertexDescriptor
