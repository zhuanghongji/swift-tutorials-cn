...
- **Alembic** : .abc - **波前对象** : .obj - **多边形** : .ply - **通用场景描述** : .usd - **标准曲面细分语言** : .stl
...
- **分配器**：分配器是负责创建包含从资源加载的网格顶点数据的缓冲区的对象。 - **描述符**：MDLVertexDescriptor 描述如何从资源加载网格顶点数据。 - **导入和导出**：MDLAsset 支持资产对象的导入和导出。有一套方法支持此功能。 - **MDLObject 子对象**：这是当你想到 MDLAsset 时想到的核心内容。它包含资源中的所有网格、相机和照明对象。
