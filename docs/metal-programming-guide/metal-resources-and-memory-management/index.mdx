...
- **newTextureWithDescriptor**：此方法在 MTLDevice 上创建新纹理。它使用 MTLTextureDescriptor 来配置纹理对象。
- **makeTextureView(pixelFormat:)**：此方法基于 MTLTexture 协议。它创建并返回一个新的纹理对象，该对象与源纹理对象共享相同的存储分配。由于它们共享相同的存储空间，因此对新纹理像素的任何更改都会反映在源纹理中，反之亦然。
- **newTextureWithDescriptor:offset:bytesPerRow**：此方法基于另一个协议 MTLBuffer。它与 makeTextureView(pixelFormat:) 类似，因为新纹理与源缓冲区对象共享相同的存储分配，因此适用相同的规则。在 newTextureWithDescriptor:offset:bytesPerRow 中，你可以根据纹理描述符创建新纹理。 MTLTextureDescriptor 配置你的新纹理对象。这包括像素格式、高度和 mipmap 级别计数。到目前为止我们讨论的有关存储模式和类型的所有方面都在这里配置。
...
- **ETC2**：ETC（爱立信纹理压缩）类似于 PVRTC。它将每个 4×4 像素块压缩为单个 64 位数量，但 PVRTC 通常质量更高。
...
处理第二种情况的过程称为 _mipmapping_。由于预计用户会放大和缩小纹理，因此 Metal 具有内置方法来生成这些 mipmap。 mipmap 背后的过程和逻辑将在第 12 章中进行更详细的解释。现在，只需了解 mipmap 是一种确保模型看起来漂亮而不是锯齿状的方法，无论它们的放大倍数是多少。
