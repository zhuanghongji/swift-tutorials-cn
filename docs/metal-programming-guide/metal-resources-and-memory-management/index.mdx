...
`func makeTextureView(pixelFormat: MTLPixelFormat) -> MTLTexture` 指定要将纹理转换为的新像素格式并返回该新纹理。并非所有像素格式都与所有其它像素格式兼容。你不能在不同大小的颜色像素格式之间使用此方法。
...
以前的方法仅允许你更改像素格式。此方法还允许你更改纹理类型。与 `MTLPixelFormat` 类似，`MTLTextureType` 具有可以将纹理转换为的有限数量的其它纹理 `type1D` 纹理只能转换为另一个 `type1D` 纹理，而 `type2D` 纹理可以转换为另一个 `type2D` 或 `type2DArray`。
