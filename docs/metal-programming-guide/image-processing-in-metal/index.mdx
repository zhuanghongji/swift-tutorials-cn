...
此方法获取图像的名称并返回 MTLTexture 对象。由于此方法可能会在某个时刻失败，因此返回类型是可选的。这是一个很长的方法，因此分为两部分。
...
与其他矩阵类似，卷积矩阵具有恒等式。单位核与单位矩阵具有相同的用途，因为如果通过单位核运行一个值，则初始值将保持不变。其工作原理是将卷积核每个框中的值乘以像素矩阵中像素的相关值。这些值全部相加即可得出操作的最终结果。假设你有一个像素值矩阵，如下所示：
...
首先要了解的是使用卷积时会受到什么影响。在上一节中，你看到一个值与其周围的其他八个值一起被采样。这有点过于简单化了。正如你在去饱和度示例中看到的那样，颜色不是由一个值表示的。它们由四个值表示：红色、绿色、蓝色的百分比和 alpha 值。当你将这些权重应用于像素及其周围的像素时，此操作将运行四次，每个颜色通道运行一次。
...
框模糊很简单，但与书中描述的其他简单程序一样，它不会产生最佳的抛光效果。使用周围像素的不等权重分布可以产生更好的效果。这种类型的模糊称为 _高斯模糊_。这是高斯模糊背后的方程：
