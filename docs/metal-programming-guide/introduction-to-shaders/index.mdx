...
<Wisdom words="颜色是我一整天的痴迷、欢乐和折磨。" author="克劳德·莫奈" />
...
矩阵遵循类似的方案。对于任何给定的矩阵，格式将为 floatNxM，其中 N 是列数，M 是行数。图形编程中最常见的矩阵类型是古老、可靠的 4×4 矩阵。在 MSL 中，这被写为 float4x4。

除了 MSL 之外，还有 Metal 标准库。其他着色语言（例如 GLSL）中存在的大多数数学函数都可以直接映射到标准库。这包括 dot()、normalize() 等。你可以将其中一些用于你的着色器。
...
着色器类型 returnType 着色器名称(参数 1, 参数 2, 等等....) { }

每个着色器必须以类型声明开始。它可以是顶点、片段或内核着色器。接下来，你需要声明你的返回类型。如果必须返回多个参数，通常为所有返回值创建结构并使返回类型成为该结构的实例。接下来，着色器需要一个名称。当你在主程序中设置渲染管道时，会访问该名称。与其他函数一样，你需要传入参数。对于顶点着色器，这些参数可以是位置、颜色或组成顶点着色器所需的其他数据位。

## 你的第一个着色器：穿越

着色器并不存在于真空中。它们需要连接到主程序，以便共享参数表。你可以创建的最简单的着色器程序从主程序接收所需的所有数据。该数据直接通过着色器而无需修改。要了解这些着色器的工作原理，你需要了解如何使用参数表连接着色器，如图 5.1 所示。

<Figure src={require("../figures/018.jpg")} num="5.1" desc="参数表" />

着色器需要从顶点缓冲区接收信息。顶点缓冲区需要分配内存并将顶点数据绑定到它。缓冲区还必须添加到参数表中，以便着色器可以访问它们。缓冲区需要在 MTKView 类中创建为实例变量，因为它们会受到几种不同方法的影响：

var vertexBuffer: MTLBuffer! = nil var vertexColorBuffer: MTLBuffer! = 零

首先，你需要指定该应用程序将使用的着色器的名称。然后，你需要生成一个管道状态描述符，其中包含你创建的 Metal 函数：

Guard let defaultLibrary = device.newDefaultLibrary() else {return} Guard letfragmentProgram = defaultLibrary.makeFunction( name: "passThroughFragment") else {return} Guard let vertexProgram = defaultLibrary.makeFunction( name: "passThroughVertex") else {return}

让 pipelineStateDescriptor = MTLRenderPipelineDescriptor() pipelineStateDescriptor.vertexFunction = vertexProgram pipelineStateDescriptor.fragmentFunction =fragmentProgram

接下来，你需要为两个缓冲区分配必要的内存并给它们一个标签。

// 生成足够大的缓冲区以允许流式传输 3 // 信号量控制帧的顶点 vertexBuffer = device.makeBuffer(length: ConstantBufferSize, options: []) vertexBuffer.label = "vertices"

let vertexColorSize = vertexData.count \* MemoryLayout.size vertexColorBuffer = device.makeBuffer( 字节: vertexColorData, 长度: vertexColorSize, 选项: []) vertexColorBuffer.label = "颜色"

最后，你需要设置顶点缓冲区。此步骤将这些缓冲区添加到主程序和着色器之间共享的参数表中。你通常在应用程序启动时预先创建缓冲区，但你将在每个帧的参数表中设置它们。你不需要每帧创建缓冲区。

renderEncoder.setVertexBuffer(vertexBuffer, offset: 256\*bufferIndex, at: 0) renderEncoder.setVertexBuffer(vertexColorBuffer, offset:0 , at: 1)
...
struct VertexInOut { float4 位置 [[位置]];浮动 4 颜色； };

顶点 VertexInOut passThroughVertex( uint vid [[vertex_id]], 常量 Packed_float4* 位置 [[buffer(0)]], 常量 Packed_float4* 颜色 [[buffer(1)]]) { VertexInOut outVertex;

outVertex.position = 位置[vid]; outVertex.color = 颜色[vid];

返回输出顶点； };

片段 half4 passThroughFragment(VertexInOut inFrag [[stage_in]]) { return half4(inFrag.color); };
...
- vid **:** 该参数为顶点 ID，连接顶点缓冲区以带入当前顶点。 - 位置**：** 该参数连接到属性表。在默认的 Metal 模板中，属性表中有两个属性：位置和颜色。位置缓冲区是第一个属性。在这里，你将当前顶点的当前位置传递给顶点着色器。 - 颜色 **：** 这是属性表中的第二个属性。

该顶点着色器创建需要返回的结构的实例。然后，它将结构体的每个成员设置为传入的参数并返回该结构体。没有处理

发生 - 着色器只是获取当前位置和颜色并将结构传递给光栅器。
...
XYZ*XYZ_XYZ_XYZ_XYZ_XYZ_XYZ_XYZ*
...
- 顶点位置 - 顶点法线 - 光源位置 - 颜色 - 反射率 - 光源强度 - 模型视图矩阵 - 投影矩阵
...
如果你熟悉其他着色语言（例如 GLSL），MSL 的设计就很容易学习。一些常见的数据类型（例如向量和矩阵）以及一些常见的函数（例如点和十字）在所有着色语言中都是必需的。然而，由于 Metal 的设计，你不能直接将 GLSL 着色器复制并粘贴到 Metal 中。

这是该项目所基于的原始 GLSL 着色器：

vec3 VertexPosition 中的布局（位置 = 0）； vec3 VertexNormal 中的布局（位置 = 1）；

输出 vec3 光强度；

均匀 vec4 LightPosition； // 眼睛坐标中的灯光位置 uniform vec3 Kd; // 漫反射率 uniform vec3 Ld; // 漫射光强度

统一 mat4 模型视图矩阵；统一 mat3 法线矩阵；统一 mat4 投影矩阵；制服 4 个 MVP；

void main() { vec3 tnorm = normalize( NormalMatrix _ VertexNormal); vec4 eyeCoords = ModelViewMatrix _ vec4(VertexPosition,1.0); vec3 s = 标准化(vec3(LightPosition - eyeCoords));

光强度 = Ld _ Kd _ max( 点( s, tnorm ), 0.0 );

gl_Position = MVP \* vec4(VertexPosition,1.0); }

查看此代码并且不熟悉 MSL，你可能会认为翻译此着色器需要在着色器文件中创建一堆常量，顶点着色器和片段着色器都将访问这些常量。这个假设是不正确的。着色器函数使用的所有数据都必须作为参数从参数表传入或硬编码到着色器本身中。

有两组参数需要传递到顶点着色器中。一组参数（顶点位置和顶点法线）是从使用模型 I/O 引入项目的模型文件中提取的。其他参数是你（程序员）负责决定的参数。这些包括光位置和表面的反射率。如果你无法在着色器程序中将它们声明为全局制服，那么顶点着色器如何访问它们？

它们的访问方式与访问位置和普通数据的方式相同：通过缓冲区。首先，你将查看视图控制器中的一些代码。当使用模型 I/O 将模型文件引入程序时，位置和法线被绑定到顶点描述符。

让 desc = MTKModelIOVertexDescriptorFromMetal(vertexDescriptor) var attribute = desc.attributes[0] as！ MDLVertexAttribute attribute.name = MDLVertexAttributePosition attribute = desc.attributes[1] as！ MDLVertexAttribute attribute.name = MDLVertexAttributeNormal Guard let mtkBufferAllocator = MTKMeshBufferAllocator(device: device) else {return}

该描述符用于设置一个顶点缓冲区，然后将其添加到参数表中：

让 vertexBuffer = mesh.vertexBuffers[0] renderEncoder.setVertexBuffer(vertexBuffer.buffer, 偏移量: vertexBuffer.offset, at: 0)

在顶点着色器中，你需要设置一个适合你刚刚创建的密钥的锁。位置和普通数据都在参数表的第一个缓冲区参数中交错。你需要在顶点着色器声明中指定该参数：

顶点 VertexOut lightingVertex(uint vid [[vertex_id]], VertexIn \*position [[buffer(0)]] { }
...
struct VertexIn { float3 位置 [[attribute(0)]]; float3 正常 [[属性(1)]]; };
...
Apple 的框架之一是 Accelerate 框架。 Accelerate 包含用于矢量和矩阵数学、数字信号处理、大数处理和图像处理的 C API。 Accelerate 中的模块之一是 SIMD 库。 SIMD 中的数据类型与你需要在 MSL 中重新创建的数据类型完美匹配。

该项目中使用了一种专门的数据类型：Matrix4x4。 SIMD 具有与 Metal float4x4 数据类型相关的 float4x4 类型，但你需要一些专门的矩阵类型，并且此自定义 Matrix4x4 数据类型包含用于创建这些类型的实例的便捷方法：

struct Matrix4x4 { var X: float4 var Y: float4 var Z: float4 var W: float4

init() { X = Vector4(x: 1, y: 0, z: 0, w: 0) Y = Vector4(x: 0, y: 1, z: 0, w: 0)

Z = Vector4(x: 0, y: 0, z: 1, w: 0) W = Vector4(x: 0, y: 0, z: 0, w: 1) }

static func spinAboutAxis(\_ axis: float4, byAngle 角度: Float32) -> Matrix4x4 { var mat = Matrix4x4()

让 c = cos(角度) 让 s = sin(角度)
...
struct Uniforms { 让 lightPosition:float4 让颜色:float4 让反射率:float3 让 lightIntensity:float3 让投影 Matrix:Matrix4x4 让 modelViewMatrix:Matrix4x4 }

**投影矩阵**

如果你完成了第 3 章“你的第一个 Metal 应用程序（你好，三角形！）”，你可能会想知道为什么现在需要投影和模型视图矩阵，而无需投影和模型视图矩阵就可以将三角形渲染到屏幕上。

如果你回想一下三角形，它并没有按照你想象的方式出现。每个顶点与其他两个顶点等距，因此你应该有一个等边三角形。但如果你构建并渲染了代码，你会注意到三角形被拉伸了。
...
// 矢量制服 let teapotColor = float4(0.7, 0.47, 0.18, 1.0) let lightPosition = float4(5.0, 5.0, 2.0, 1.0) let Reflectivity = float3(0.9, 0.5, 0.3) lettensity = float3(1.0, 1.0, 1.0)

// 矩阵统一 let yAxis = float4(0, -1, 0, 0) var modelViewMatrix = Matrix4x4.rotationAboutAxis(yAxis, byAngle:rotationAngle)

modelViewMatrix.W.z = -2

让方面 = Float32(self.view.bounds.width) / Float32(self.view.bounds.height)
...
让 uniform = Uniforms（lightPosition：lightPosition，颜色：teapotColor，反射率：反射率，lightIntensity：强度，projectionMatrix：projectionMatrix，modelViewMatrix：modelViewMatrix）

让 uniforms = [uniform]uniformBuffer = device.makeBuffer（长度：MemoryLayout.size，选项：[]）memcpy（uniformBuffer.contents（），uniforms，MemoryLayout.size）

在着色器程序中，正如你对位置数据所做的那样，你需要创建一个与 Swift 结构中的数据类型相关的结构：

struct Uniforms { float4 lightPosition; }浮动 4 颜色； Packed_float3 反射率； Packed_float3 强度； float4x4 模型视图矩阵； float4x4 投影矩阵； };

最后，将此缓冲区作为参数添加到片段着色器：

顶点 VertexOut lightingVertex( VertexIn vertexIn [[stage_in]], 常量 Uniforms &uniforms [[buffer(1)]]) { }
...
顶点 VertexOut lightingVertex( VertexIn vertexIn [[stage_in]], 常量 Uniforms &uniforms [[buffer(1)]]) { VertexOut outVertex;

// 光照代码 float3 tnorm = normalize(uniforms.projectionMatrix _ vertexIn.normal); float4 eyeCoords = Uniforms.modelViewMatrix _ float4(vertexIn.position, 1.0); float3 s = 标准化(float3(uniforms.lightPosition – eyeCoords));

outVertex.lightIntensity = uniforms.intensity _ uniforms.reflectivity _ max( dot(s, tnorm), 0.0); outVertex.position = uniforms.modelViewMatrix \* float4(vertexIn.position, 1.0);
...
片段 half4 lightingFragment( VertexOut inFrag [[stage_in]], 常量 Uniforms &uniforms [[buffer(1)]]) { return half4(inFrag.lightIntensity \*uniforms.color); };
