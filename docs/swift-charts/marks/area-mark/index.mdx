...
初始化程序的行为类似于 [init(x:y:stacking:)](<https://developer.apple.com/documentation/charts/areamark/init(x:y:stacking:)>)，不同之处在于你可以通过为 `series` 输入提供值来指示每个数据点属于哪个区域。这使你能够在一张图表上绘制多个区域。
...
使用此初始值设定项创建具有垂直间隔的范围面积图。例如，你可以创建一个包含一天中、多天内的所有温度的区域：
...
初始化程序的行为类似于 [init(x:yStart:yEnd:)](<https://developer.apple.com/documentation/charts/areamark/init(x:ystart:yend:)>)，不同之处在于你可以通过为 series 输入提供值来指示每个间隔属于哪个区域。这使你能够在一张图表上绘制多个区域。
...
使用此初始值设定项创建具有水平间隔的范围面积图。例如，你可以创建一个包含一天中、多天内的所有温度的区域：
...
初始化程序的行为类似于 [init(xStart:xEnd:y:)](<https://developer.apple.com/documentation/charts/areamark/init(xstart:xend:y:)>)，不同之处在于你可以通过为 series 输入提供值来指示每个间隔属于哪个区域。这使你能够在一张图表上绘制多个区域。
