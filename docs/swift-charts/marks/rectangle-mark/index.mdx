...
使用此初始值设定项将每个数据元素的 y 起点、y 终点和 x 位置映射到矩形。 （可选）指定矩形的宽度。
...
使用此初始值设定项将每个数据元素的 x 起点、x 终点和 y 位置映射到矩形。 （可选）指定矩形的高度。
...
使用此初始值设定项将每个数据元素的 x 开始、x 结束、y 开始和 y 结束位置映射到矩形。
...
使用此初始值设定项将每个数据元素的 x 和 y 位置映射到矩形。 （可选）指定矩形的宽度或高度。
...
使用此初始值设定项将每个数据元素的 x 位置映射到矩形。 （可选）指定矩形的宽度、yStart 位置或 yEnd 位置。
...
使用此初始值设定项将每个数据元素的 y 位置映射到矩形。 （可选）指定矩形的高度、xStart 位置或 xEnd 位置。
...
使用此初始值设定项可创建一个具有任意可选 x 起始位置、x 结束位置、y 起始位置和 y 结束位置的矩形。如果未指定参数，则矩形将填充图表的绘图区域。
...
使用此初始值设定项将每个数据元素的 y 开始和 y 结束位置映射到矩形。 （可选）指定矩形的 xStart 或 xEnd 位置。
...
使用此初始值设定项将每个数据元素的 x 起点和 x 终点映射到矩形。 （可选）指定矩形的 yStart 或 yEnd 位置。
