...
`ChartContent` 协议提供了一组修饰符，可用于配置图表内容的属性。它们的行为类似于 SwiftUI 视图修饰符，只不过它们作用于图表内容而不是视图。任何符合该协议的类型都可以使用这些修饰符。例如，你可以将 [foregroundStyle(\_:)](<https://developer.apple.com/documentation/charts/chartcontent/foregroundstyle(_:)>) 修饰符添加到上一示例中表示球体（spheres）数量的条形图上，使其变为红色：
