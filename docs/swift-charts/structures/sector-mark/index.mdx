...
为了确保可视化易于阅读，请设计不超过 5-7 个扇区的饼图或圆环图。如有必要，将所有剩余值汇总到“其他”组中，或者考虑水平条形图，它可以扩展到许多条形，易于用类别标记，并让用户更准确地比较项目。

确保你的数据仅包含正值。此外，图表中非常小的比例可能无法辨别，特别是在指定了角度插入的情况下。

## 创建带有扇形标记的饼图

要直观地显示值与它们总共加起来的总数的比率，请指定值，通常按递减的值排序。如果需要，添加“其他”组作为最后一项。
