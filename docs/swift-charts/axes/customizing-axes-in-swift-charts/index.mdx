...
使用 chartYScale(domain:type:) 视图修饰符设置 y 轴的范围：
...
Swift Charts 选择默认数量的网格线和标签来显示在每个轴上。你可以使用 chartYAxis(content:) 修饰符指定不同数量的值，该修饰符在其 content 参数中接受一个或多个 AxisMarks 实例。通过使用 automatic(desiredCount:roundLowerBound:roundUpperBound:) 值初始化轴标记实例来请求特定数量的值。例如，你可以请求轴具有三个值：
...
你可以通过正确设置图表的轴值格式来增加图表的清晰度。电池图表中 y 轴上的值表示百分比，因此你可以将 format 参数传递给轴标记初始值设定项以应用适当的格式：
...
要添加更复杂的样式，请在修饰符的 content 闭包内编写轴标记。例如，要添加更多水平网格线而不添加更多标签，请使用两个单独的 AxisMarks 实例分别渲染网格线和标签：
...
上述代码中的 AxisMarks 初始值设定项均采用一个 content 参数，该参数是一个轴构建器，指示标记适用于哪些元素。使用构建器组合 AxisGridLine 、 AxisTick 和 AxisValueLabel 元素。上面的示例在 0 、 25 、 50 、 75 和 100 处渲染网格线，但将仅在 0 、 50 和 100 处添加标签：

<Figure src={require("./image_6.png")} size={420} />

对于其他自定义，请应用 AxisMark 提供的样式方法。例如，你可以通过将 font(\_:) 方法应用于 AxisValueLabel 实例来调整值标签的字体。

## 有条件地设置标签格式

对于指定的数据，电池图表沿 x 轴标记每六小时一次，并且默认情况下包括每小时的 AM 或 PM 指示器。你可以通过每三小时添加一个标记并仅在 AM 或 PM 指示器发生变化时显示该指示器来使图表更具可读性。

使用 chartXAxis(content:) 视图修饰符来配置 x 轴，就像使用 chartYAxis(content:) 修饰符配置 y 轴一样。以下代码将 stride(by:count:roundLowerBound:roundUpperBound:calendar:) 值传递给 AxisMarks 实例来控制标记的频率，并使用条件格式仅在中午和午夜显示 AM 或 PM：
