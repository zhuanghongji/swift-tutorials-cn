...
iOS 应用程序自动参与 AirPods 自动切换。要使您的 macOS 应用参与此行为，请使用 AVAudioRoutingArbiter 指示您的应用何时开始以及完成播放或录制音频。例如，IP 语音 (VoIP) 应用程序可能会在开始呼叫之前请求仲裁，并在仲裁完成后开始 VoIP 会话。同样，当通话结束时，应用程序将结束 VoIP 会话并离开仲裁。
