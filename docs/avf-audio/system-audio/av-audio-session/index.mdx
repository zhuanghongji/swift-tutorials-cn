...
你可以使用六种可能的类别，但 playback 是播放应用程序最常使用的类别。此类别表明音频播放是你应用程序的核心功能。当你指定此类别时，你的应用程序的音频将继续，并将响铃/静音开关设置为静音模式（仅限 iOS）。如果你使用音频、AirPlay 和画中画背景模式，则还可以使用此类别播放背景音频。有关详细信息，请参阅 Enabling Background Audio 。

你可以使用 AVAudioSession 对象来配置应用程序的音频会话。此类是一个单例对象，用于设置音频会话的类别、模式和其他配置。你可以在应用程序的整个生命周期中与音频会话进行交互，但在应用程序启动时执行此配置通常很有用，如以下示例所示。
...
你可以在设置其类别后随时激活音频会话，但通常最好推迟此调用，直到你的应用程序开始音频播放。推迟通话可确保你不会过早中断可能正在进行的任何其他背景音频。
