...
public struct Query<Element, Result> : DynamicProperty where Element : PersistentModel {
    // ...
}
```
...
//
// - descriptor: 一个 `SwiftData.FetchDescriptor`
// - animation: 用于因获取结果的更改而导致的用户界面更改的动画
public init(_ descriptor: FetchDescriptor<Element>,
            animation: Animation
) where Result == [Element]
```
...
```swift
// 创建一个带有谓词、用于排序的属性的关键路径以及排序依据的查询
//
// - filter: Element 上的谓词
// - sort: 用于排序的属性的键路径
// - order: 是否按正序或倒序排序
