...
应用程序将此断言应用于它动态创建的查询，如下一节所述。通过在中心位置定义一次断言，多个视图中的查询就可以使用它。当视图具有不同的查询时，这使得同步相关视图（例如列表视图和地图视图）变得容易。
...
由于这些值是视图初始化器的输入，因此每当任一值发生更改时，SwiftUI 都会重新评估初始化器以生成新的查询。这反过来会更新视图的外观。
