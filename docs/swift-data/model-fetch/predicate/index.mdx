...
断言是计算结果为布尔值（真或假）的逻辑条件。你可以使用断言进行过滤集合或搜索匹配元素等操作。
...
你可以安全地对断言进行编码和解码、跨并发边界传递断言以及从文件加载断言。要定义读取存档断言时允许的类型和关键路径列表，请使用 PredicateCodableConfiguration 。

你可以使用 expression 属性将断言转换为另一种表示形式 - 例如，以另一种查询语言表示断言，或创建修改后的断言。
