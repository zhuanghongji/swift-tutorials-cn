...
此示例项目旨在帮助你了解如何在现有 Core Data 应用程序中采用 SwiftData。SampleTrips 应用程序从存储（store）中获取并显示所有即将到来的旅行（upcoming trips），并允许人们创建或删除旅行，以及在每次旅行的行程（itinerary）中添加、更新或删除信息。这个应用程序有三个版本：
...
此示例中的每个模型文件都使用 [Model()](<https://developer.apple.com/documentation/swiftdata/model()>) 宏为 `PersistentModel` 和 [Observable](https://developer.apple.com/documentation/observation/observable) 协议添加必要的一致性：
...
此示例应用程序通过将旅行数组包装在 [Query](https://developer.apple.com/documentation/swiftdata/query) 中来获取即将到来的旅行的完整列表，该数组从容器中获取 `Trip` 对象。
...
- 此示例应用程序使用「应用程序组」来访问共享容器并在 "SwiftData 小部件扩展" 和 "Core Data 宿主应用程序" 之间共享数据。对于具有 App Groups Entitlement 的应用程序，它将数据存储持久到应用程序组容器的根目录。对于从没有任何「应用程序组」容器的版本演变为具有应用程序组容器的版本的应用程序，SwiftData 会将现有存储复制到应用程序组容器。

在此示例中，主应用程序和小部件通过「应用程序组」容器共享同一存储，并且该存储位于应用程序组容器中的默认位置。为了确保 SwiftData 访问同一个存储，主应用程序和小部件都共享 [ModelContainer](https://developer.apple.com/documentation/swiftdata/modelcontainer)。
