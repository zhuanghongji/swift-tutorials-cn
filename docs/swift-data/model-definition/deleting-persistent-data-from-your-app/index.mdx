...
但是，使用此示例的人可能会删除选定的动物，因此 `removeAnimals` 方法需要在删除动物之前取消选择该动物。`removeAnimals` 的更新版本使用 [persistentModelID](https://developer.apple.com/documentation/swiftdata/persistentmodel/persistentmodelid) 来确定要删除的动物是否也是所选动物。如果是，该方法将选定的动物设置为 `nil`。
