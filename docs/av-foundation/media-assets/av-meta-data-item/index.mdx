...
要在第一次访问元数据项时加载元数据项的值，请使用 AVAsynchronousKeyValueLoading 协议中的方法。 AVAsset 类和其他类依次根据需要提供其元数据，以便你可以从这些数组中获取对象，而不会产生不检查的项目的开销。

要过滤元数据项的数组，你可以使用此类的方法。例如，你可以按键和键空间、区域设置以及首选语言进行过滤。
