...
媒体播放应用程序通常需要监视播放进度以驱动播放器 UI 的状态或执行其他操作。监视此状态需要比键值观察所能提供的更高级别的时间精度，因此 AVPlayer 提供了特定的 API 来观察播放时间。本文介绍了如何定期观察此状态或在播放跨越特定时间边界时观察此状态。
...
观察播放器的另一种方法是播放器在播放过程中何时跨越特定时间边界。你可以通过更新播放器 UI 或执行其他操作来应对这些时间的流逝。
