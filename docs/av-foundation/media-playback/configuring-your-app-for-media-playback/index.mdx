...
使用 AVAudioSession 对象来配置应用程序的音频会话。音频会话是一个单例对象，用于设置音频会话 category 、 mode 和其他设置。要配置音频会话以优化电影播放：
...
你可以在设置其类别后随时激活音频会话，但建议推迟此调用，直到你的应用程序开始音频播放。推迟通话可确保你不会过早中断可能正在进行的任何其他背景音频。

:::

设置类别是与音频会话的最小交互，但其他配置选项和功能也可用。例如，在 visionOS 中，你可以通过配置音频会话来自定义用户的空间音频体验。有关详细信息，请参阅 AVAudioSession 。
