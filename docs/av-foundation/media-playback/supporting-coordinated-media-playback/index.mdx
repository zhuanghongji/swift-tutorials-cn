...
此示例应用程序向你展示如何向你的应用程序添加协调的媒体播放支持。它提供了一个简单的电影播放应用程序，用户可以从库中选择电影并在标准播放器用户界面中播放。
...
该示例启用群组播放的最后一步是访问播放器的协调器并将其与群组会话连接。它通过调用协调器对象的 coordinateWithSession(\_:) 方法来实现此目的，该方法将其与会话中其他参与者的协调器连接起来。
...
样本建立此连接后，系统会协调参与玩家之间的速率和时间变化。该应用程序继续以典型方式对电影进行排队，并使用播放器的标准传输方法（如 play() 、 pause() 和 seek(to:completionHandler:) ）控制播放。协调器拦截这些更改并根据需要将它们传达给其他协调器。同样，它还响应其他参与者的速率和时间变化，并将它们设置在本地播放器上。

## 暂停参与协调播放 in page link

在大多数情况下，样本会与组保持播放同步。然而，有时需要防止本地中断影响其他参与者。在这些情况下，它会通过发出播放暂停来暂时断开与组的连接。 AVPlayerPlaybackCoordinator 自动针对常见系统事件（例如网络停滞和音频会话中断）发出播放暂停。应用程序还可以定义自定义暂停。
...
当用户点击用户界面中的功能按钮时，示例将调用其 performWhatHappened() 方法。在此方法中，它启动自定义暂停，快退 10 秒，然后以双倍速度播放，直到播放赶上该组。
