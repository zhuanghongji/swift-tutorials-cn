...
作为开发人员，你希望让尽可能广泛的受众可以访问你的应用程序。扩大应用程序覆盖范围的一种方法是，以用户的母语向用户提供该应用程序，并为有听力障碍或有其他辅助功能需求的用户提供支持。 AVKit 和 AVFoundation 通过提供对显示字幕和隐藏式字幕以及选择替代音频和视频轨道的内置支持，简化了这些问题的处理。如果你正在构建自己的自定义播放器或想要呈现自己的媒体选择界面，请使用 AVFoundation 的 AVMediaSelectionGroup 和 AVMediaSelectionOption 类提供的功能。
