...
在此示例中，你将学习所有 Metal 应用程序中使用的基本任务。你将看到如何将用 C 编写的简单函数转换为 Metal 着色语言（Metal Shading Language / MSL），以便它可以在 GPU 上运行。你将找到一个 GPU，通过创建管道（pipeline）来准备要在其上运行的 MSL 函数，并创建 GPU 可访问的数据对象。要针对数据执行管道，请创建命令缓冲区（command buffer），将命令（commands）写入其中，然后将缓冲区提交到命令队列（command queue）。Metal 将命令发送到 GPU 来执行。
...
每个值都是独立计算的，因此可以安全地同时（concurrently）计算这些值。要在 GPU 上执行计算，需要用 Metal Shading Language（MSL）重写此函数。MSL 是 C++ 的变体（variant），专为 GPU 编程而设计。在 Metal 中，在 GPU 上运行的代码称为 **着色器（shader）**，因为历史上它们首先用于计算 3D 图形中的颜色。清单 2 显示了 MSL 中的着色器，它执行与清单 1 相同的计算。示例项目在 add.metal 文件中定义了此函数。Xcode 在应用程序目标中构建所有 .metal 文件，并创建一个默认的 Metal 库（default Metal library），且将其嵌入到你的应用程序中。稍后，你将在本示例中看到如何加载默认库。
...
- 公共（public）GPU 函数。公共函数是你的应用程序可以看到的唯一函数。公共函数也不能被其他着色器函数调用。
...
清单 2 从清单 1 中删除了 `for` 循环，因为该函数现在将由计算网格（compute grid）中的多个线程调用。此示例创建一个与数组维度完全匹配的一维线程网格，以便数组中的每个条目都由不同的线程计算。
...
Metal 将其他与 GPU 相关的实体（entities），比如编译的着色器（compiled shaders）、内存缓冲区（memory buffers）和纹理（textures），表示为对象。要创建这些特定于 GPU 的对象，你可以调用 [MTLDevice](https://developer.apple.com/documentation/metal/mtldevice) 上的方法，或者调用 [MTLDevice](https://developer.apple.com/documentation/metal/mtldevice) 创建的对象上的方法。由设备对象直接或间接创建的所有对象只能与该设备对象一起使用。使用多个 GPU 的应用程序将使用多个设备对象，并为每个设备对象创建类似的 Metal 对象层次结构。

示例应用程序使用自定义 `MetalAdder` 类来管理与 GPU 通信所需的对象。类的初始化器创建这些对象并将它们存储在其属性中。应用程序创建此类的实例，并传入 Metal 设备对象以用于创建辅助对象。`MetalAdder` 对象保持对 Metal 对象的强引用，直到它完成执行。
...
该函数对象是 MSL 函数的代理，但它不是可执行代码。你可以通过创建 **管道（pipeline）**将函数转换为可执行代码。管道指定 GPU 为完成特定任务而执行的步骤。在 Metal 中，管道由 **管道状态对象（pipeline state object）** 表示。由于此示例使用计算函数，因此应用程序创建一个 [MTLComputePipelineState](https://developer.apple.com/documentation/metal/mtlcomputepipelinestate) 对象。
...
要将命令写入命令缓冲区，你可以使用 **命令编码器（command encoder）**来编码你想要编码的特定类型的命令。此示例创建一个计算命令编码器，用于对 **计算通道（compute pass）**进行编码。计算通道包含执行计算管道的命令列表。每个计算命令都会导致 GPU 创建一个线程网格以在 GPU 上执行。
...
接下来，决定要创建多少个线程以及如何组织这些线程。Metal 可以创建一维、二维或三维网格。`add_arrays` 函数使用一维数组，因此示例创建大小为（`dataSize` x `1` x `1`）的一维网格，Metal 从中生成 `0` 到 `dataSize` 的索引。
...
在命令缓冲区完成后，GPU 的计算结果将存储在输出缓冲区中，Metal 会执行任何必要的步骤以确保 CPU 可以看到它们。在真实的应用程序中，你将从缓冲区读取结果并对其执行某些操作，例如在屏幕上显示结果或将其写入文件。由于计算仅用于阐明创建 Metal 应用程序的过程，因此示例读取输出缓冲区中存储的值并进行测试，以确保 CPU 和 GPU 计算出相同的结果。
