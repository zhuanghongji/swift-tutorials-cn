...
这个方法告诉 Metal，当命令缓冲区被安排执行时，Metal 应该与 Core Animation 协调以在渲染完成后显示纹理。当 Core Animation 呈现纹理时，它就成为视图的新内容。在此示例中，这意味着擦除的纹理将成为视图的新背景。这一变化与 Core Animation 为屏幕用户界面元素所做的任何其它视觉更新一起发生。
