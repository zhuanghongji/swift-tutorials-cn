...
你还可以选择通过调用命令缓冲区的 [enqueue()](https://developer.apple.com/documentation/metal/mtlcommandbuffer/1443019-enqueue) 方法在其命令队列中为命令缓冲区保留一个位置。在将缓冲区提交到队列之前，你可以随时调用该方法一次。如果你没有将命令缓冲区排入队列，那么当你提交命令缓冲区时，它会隐式地将其自身排入队列。每个命令队列都确保将其命令缓冲区排入队列的顺序与队列安排它们在 GPU 上运行的顺序相同。
