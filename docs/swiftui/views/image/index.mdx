...
SwiftUI 用前景样式填充第一层，其他图层填充前景样式的二级和第三层变体。

可以使用 `foregroundStyle(_:_:)` 和 `foregroundStyle(_:_:_:)` 修饰器显式指定这些样式。如果只指定主前景样式，SwiftUI 会自动从该样式派生（derives）其他样式。例如，可以渲染填充感叹号三角形，其中紫色作为感叹号的色调颜色，将较低的不透明度紫色作为三角形：
...
可以使用 `foregroundStyle(_:_:)` 和 `foregroundStyle(_:_:_:)` 修饰器显式指定这些样式。如果只指定主前景样式，SwiftUI 会自动从该样式派生其他样式。例如，可以用黄色作为感叹号的色调颜色渲染填充感叹号三角形，然后用青色填充三角形：
