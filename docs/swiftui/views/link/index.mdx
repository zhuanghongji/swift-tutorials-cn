...
与其他视图一样，你可以使用标准视图修饰符设置链接样式，具体取决于链接标签的视图类型。例如，可以使用自定义 `font(_:)` 或 `foregroundColor(_:)` 修改 `Text` 标签，以自定义链接在应用 UI 中的外观。
...
> "gz_tower" 是在 [Image](../image/) 一文中添加到 Assets 的图片资源。
