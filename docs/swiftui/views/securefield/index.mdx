...
<OriginalLink
  title="用户在其中安全地输入私人文本的控件"
  value="https://developer.apple.com/documentation/swiftui/securefield"
/>

## 概述

在你希望行为类似于 `TextField` 但不希望用户的文本可见，请使用 `SecureField`。通常，你可以使用它来输入密码和其他敏感信息。

一个 `SecureField` 使用一个字符串值的绑定，以及一个用户提交编辑时执行的闭包，例如按 _Return_ 键。该输入框在每次按键（keystroke）或其他编辑时更新绑定字符串，因此你可以随时从另一个控件（如 "完成" 按钮）读取其值。
