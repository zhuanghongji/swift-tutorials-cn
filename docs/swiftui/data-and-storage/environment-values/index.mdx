...
<OriginalLink
  title="使用环境在整个视图层次结构中共享数据"
  value="https://developer.apple.com/documentation/swiftui/environment-values"
/>
...
你可以使用此属性包装器来读取（但不能设置）环境值。 SwiftUI 根据系统设置自动更新一些环境值，并为其他环境值提供合理的默认值。你可以使用 `environment(_:_:)` 视图修饰符覆盖其中一些，也可以设置你定义的 "自定义环境值"。
